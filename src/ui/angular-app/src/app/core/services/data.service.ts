...
import {
  Company, ConsumptionSite, BarycentreResult, DashboardSummary,
  CreateSitePayload, UpdateSitePayload, CreateCompanyPayload, UpdateCompanyPayload,
  CalculateRequest, AlgorithmType
} from '../models';

// =============================================================================
// DataService — in-memory state management (no backend required)
// Mirrors the Java in-memory repositories and sample data seeder.
// Replace individual methods with HTTP calls when a REST API is available.
// =============================================================================
...
export class DataService {
  ...
  private _sites      = signal<ConsumptionSite[]>([]);
  private _results    = signal<BarycentreResult[]>([]);
  ...
  readonly activeCompanies = computed(() =>
    this._companies().filter(c => c.status === 'ACTIVE')
  );

  readonly activeSites = computed(() =>
    this._sites().filter(s => s.status === 'ACTIVE')
  );
  ...
  readonly dashboardSummary = computed<DashboardSummary>(() => {
    ...
    const totalTons = sites.filter(s => s.status === 'ACTIVE')
  ...
  createCompany(payload: CreateCompanyPayload): Company {
  ...
  updateCompany(id: string, payload: UpdateCompanyPayload): Company | null {
    let updated: Company | null = null;
    this._companies.update(list => list.map(c => {
      if (c.id !== id) return c;
      updated = { ...c, ...payload, updatedAt: now() };
      return updated!;
    }));
  ...
  getSitesByCompany(companyId: string): ConsumptionSite[] {
    return this._sites().filter(s => s.companyId === companyId);
  }

  getResultsByCompany(companyId: string): BarycentreResult[] {
  ...
  createSite(payload: CreateSitePayload): ConsumptionSite {
    const site: ConsumptionSite = {
      id: uid('STE'),
  ...
  updateSite(id: string, payload: UpdateSitePayload): ConsumptionSite | null {
    let updated: ConsumptionSite | null = null;
    ...
    if (updated) this.recomputeCompanyStats((updated as ConsumptionSite).companyId);
    return updated;
  }
  ...
  deleteSite(id: string): void {
    ...
    if (site) this.recomputeCompanyStats(site.companyId);
  }

  getSiteById(id: string): ConsumptionSite | undefined {
  ...
  calculate(request: CalculateRequest): BarycentreResult {
    const sites = this._sites().filter(
      s => s.companyId === request.companyId && s.status === 'ACTIVE'
    );
    ...
    let convergenceErrorKm: number;

    if (request.algorithm === 'weiszfeld-iterative') {
      const r = this.weiszfeldIterate(
        lats, lons, weights,
        request.maxIterations ?? 1000,
        request.toleranceKm  ?? 0.01
      );
      optLat             = r.lat;
      optLon             = r.lon;
  ...
  updateResultStatus(id: string, status: BarycentreResult['status']): void {
    this._results.update(list =>
      list.map(r => r.logisticsCenterId === id ? { ...r, status } : r)
    );
  }
  ...
  private weightedBarycentre(
    lats: number[], lons: number[], weights: number[]
  ...
  private weiszfeldIterate(
    lats: number[], lons: number[], weights: number[],
    maxIter: number, toleranceKm: number
  ): { lat: number; lon: number; iterations: number; error: number } {
  ...
  private recomputeCompanyStats(companyId: string): void {
    const companySites = this._sites().filter(
  ...
  private seedSampleData(): void {
    ...
    const seedCompanies: Company[] = [
      {
        id: acmeId, name: 'Acme Corp', type: 'SHIPPER', status: 'ACTIVE',
        taxId: '12-3456789', contactName: 'Jane Smith',
        contactEmail: 'j@acme.com', contactPhone: '+1-555-0100',
        consumptionSiteCount: 4, totalTrafficTons: 1205,
        createdAt: '2024-01-15T09:00:00Z', updatedAt: '2024-03-01T14:30:00Z',
      },
      {
        id: freightId, name: 'Global Freight', type: 'CARRIER', status: 'ACTIVE',
        taxId: '98-7654321', contactName: 'Bob Johnson',
        contactEmail: 'b@globalfreight.com', contactPhone: '+1-555-0200',
        consumptionSiteCount: 3, totalTrafficTons: 1720,
        createdAt: '2024-01-20T10:00:00Z', updatedAt: '2024-02-28T11:15:00Z',
      },
      {
        id: fastLogId, name: 'FastLog Inc', type: 'BOTH', status: 'ACTIVE',
        taxId: '55-1122334', contactName: 'Maria Garcia',
        contactEmail: 'm@fastlog.com', contactPhone: '+1-555-0300',
        consumptionSiteCount: 3, totalTrafficTons: 1240,
        createdAt: '2024-02-01T08:00:00Z', updatedAt: '2024-03-05T09:45:00Z',
      },
      {
        id: 'CMP-004', name: 'BayArea Transport', type: 'CARRIER', status: 'ACTIVE',
        consumptionSiteCount: 0, totalTrafficTons: 0,
        createdAt: '2024-02-10T11:00:00Z', updatedAt: '2024-02-10T11:00:00Z',
      },
      {
        id: 'CMP-005', name: 'Pacific Shipping', type: 'SHIPPER', status: 'INACTIVE',
        consumptionSiteCount: 0, totalTrafficTons: 0,
        createdAt: '2024-01-05T09:00:00Z', updatedAt: '2024-02-20T16:00:00Z',
      },
      {
        id: 'CMP-006', name: 'Metro Moves', type: 'BOTH', status: 'ACTIVE',
        consumptionSiteCount: 0, totalTrafficTons: 0,
        createdAt: '2024-03-01T10:00:00Z', updatedAt: '2024-03-01T10:00:00Z',
      },
      {
        id: 'CMP-007', name: 'Apex Logistics', type: 'CARRIER', status: 'PENDING',
        consumptionSiteCount: 0, totalTrafficTons: 0,
        createdAt: '2024-03-08T14:00:00Z', updatedAt: '2024-03-08T14:00:00Z',
      },
      {
        id: 'CMP-008', name: 'SkyBridge Co', type: 'SHIPPER', status: 'ACTIVE',
        consumptionSiteCount: 0, totalTrafficTons: 0,
        createdAt: '2024-03-10T09:30:00Z', updatedAt: '2024-03-10T09:30:00Z',
      },
    ];

    const seedSites: ConsumptionSite[] = [
      // Acme Corp (CMP-001)
      ...
      { id: 'STE-002', companyId: acmeId,    name: 'East Hub',          description: 'East Coast distribution',  latitude: 40.7128,  longitude: -74.0060,  formattedCoordinate: '40.7128, -74.0060',  weightTons: 380, weightFormatted: '380 t', address: '1 World Trade Center', city: 'New York', country: 'USA', status: 'ACTIVE', createdAt: '2024-01-16T09:05:00Z' },
...
// ---------------------------------------------------------------------------
// Haversine distance helper (mirrors GeoCoordinate.distanceKmTo)
// ---------------------------------------------------------------------------
